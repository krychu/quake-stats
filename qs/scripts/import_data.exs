defmodule Test do
  def main() do
    ...
    [pg_host, pg_port, pg_user, pg_password, pg_db, sampledata_path] = System.argv();
    {pg_port, ""} = Integer.parse(pg_port)

    {:ok, pg_pid} = pg_get(pg_host, pg_port, pg_user, pg_password, pg_db)

    results = get_filenames(sampledata_path)
    |> Enum.with_index(0)
  ...
  def get_filenames(sampledata_path) do
    Path.wildcard(sampledata_path <> "/*.json")
  end
  ...
  def pg_get(pg_host, pg_port, pg_user, pg_password, pg_db) do
    # Postgrex.start_link(hostname: "192.168.1.106", port: 32768, username: "postgres", database: "quakestats")
    Postgrex.start_link(hostname: pg_host, port: pg_port, username: pg_user, password: pg_password, database: pg_db)
  end
