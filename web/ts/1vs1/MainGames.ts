...
const commands: [string, cmd.Cmd][] = [
    [ "[1_main] fetch_games",         cmd_fetch_games ],
    [ "[1_main] create_games_root",   cmd_create_root ],
    [ "[1_main] attach_games_root",   cmd_attach_root ],
    [ "[1_main] render_games",        cmd_render ]
];

export function init() {
    cmd.add_cmds(commands);
    const substate: MainGames = {
        show_game_cnt: 20,
        html_root: null
        // sort_column_name: "ago",
        // sort_direction: "desc"
    };
    state.duel_players.games = substate;
    //state.duel_players.data.games;
...
// Commands
//------------------------------------------------------------------------------
async function cmd_fetch_games(): Promise<void> {
    if (!state.header?.time_period || !state.duel_players.data) {
        log.log("MainGames.ts - cmd_fetch_games - wrong state");
        return Promise.reject();
    }

    return fetch(_api_url_games(state.duel_players.data.game_cnt), {
        method: "POST",
...
function _api_url_games(game_cnt: number): string {
    return `/api/1vs1/main/games/${game_cnt}`;
}
